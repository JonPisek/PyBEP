...
def perform_optimization(cathode_number, cathode_info, anode_number, anode_info, OCV_battery, SOC_battery):
    """
    ...
    opt_result = differential_evolution(
        lambda params: optimization(params, anode_interp, anode_x_values, cathode_interp, cathode_x_values, OCV_battery, SOC_battery),
        bounds
    )
...
def perform_full_optimization(SOC_battery, OCV_battery, interpolated_cathodes, interpolated_anodes, iterations=5, battery=1, anode=0, cathode=0, derivative_inverse=0):
    """
    Perform full optimization for multiple iterations and find the overall best optimization result.
    ...
    for iteration in range(iterations):
        ...
        for cathode_number, cathode_info in interpolated_cathodes.items():
            for anode_number, anode_info in interpolated_anodes.items():
                result = perform_optimization(cathode_number, cathode_info, anode_number, anode_info, OCV_battery, SOC_battery)
                optimization_results.append(result)

        best_optimization_result_iteration = min(optimization_results, key=lambda x: x['RMSD'])
...
def perform_full_optimization_parallel(SOC_battery, OCV_battery, interpolated_cathodes, interpolated_anodes, iterations=5, battery=1, anode=0, cathode=0, derivative_inverse=0):
    """
    ...
    def optimize_single_case(cathode_number, cathode_info, anode_number, anode_info):
        return perform_optimization(cathode_number, cathode_info, anode_number, anode_info, OCV_battery, SOC_battery)
    ...
    best_optimization_results.extend(optimization_results)
